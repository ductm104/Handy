...
pub async fn transcribe_file(
    ...
    emit_file_transcription_progress(&app, "decoding", None, Some(0));
    ...
    const DECODE_CHUNK_SIZE: usize = 16_000; // 1 second of 16 kHz audio

    let tm = Arc::clone(&transcription_manager);
    let progress_app = app.clone();
    let decode_progress_app = app.clone();
    ...
    let streaming_result =
        tauri::async_runtime::spawn_blocking(move || -> Result<String, String> {
            ...
            let make_callback =
                |chunk_accumulated: Arc<std::sync::Mutex<String>>| -> TranscriptionProgressCallback {
                ...
                .map_err(|e| format!("Failed to decode media file: {}", e))?
            {
                ...
                decoded_samples += audio_chunk.len();

                if let Some(total) = total_samples_estimate {
                    if total > 0 {
                        let progress =
                            ((decoded_samples as f64 / total as f64) * 50.0).min(50.0) as i32;
                        emit_file_transcription_progress(
                            &decode_progress_app,
                            "decoding",
                            None,
                ...
                if prev_in_speech && !in_speech && !current_chunk.is_empty() {
                    let callback = make_callback(Arc::clone(&accumulated_text));
                    let text = tm
