...
pub fn get_icon_path(theme: AppTheme, state: TrayIconState) -> &'static str {
    match (theme, state) {
        ...
        (AppTheme::Colored, TrayIconState::Idle) => "resources/hanhcute.png",
...
fn version_label() -> String {
    if cfg!(debug_assertions) {
        format!("HanhCute v{} (Dev)", env!("CARGO_PKG_VERSION"))
    } else {
        format!("HanhCute v{}", env!("CARGO_PKG_VERSION"))
    }
}
...
mod tests {
    ...
    fn build_entry(transcription: &str, post_processed: Option<&str>) -> HistoryEntry {
        HistoryEntry {
            id: 1,
            file_name: "hanhcute-1.wav".to_string(),
            timestamp: 0,
            saved: false,
